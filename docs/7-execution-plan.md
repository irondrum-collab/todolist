...
**작업 항목**
- [x] 회원가입 → 로그인 → 할 일 등록 → 목록 확인 → 수정 → 삭제 전체 흐름 수동 테스트
- [x] 필터 기능 (카테고리·상태) 동작 확인
- [x] 내 정보 수정 (이름·비밀번호) 동작 확인
- [x] 375px (모바일) / 768px (태블릿) / 1440px (데스크톱) 반응형 레이아웃 확인
- [x] 미인증 접근 라우트 보호 확인
- [x] JWT 만료 시 로그인 페이지 이동 확인 (코드 검증: 401 인터셉터)

**완료 조건**
- [x] 전체 유스케이스 UC-01 ~ UC-07 정상 동작 확인
- [x] 콘솔 오류 없음
- [x] 반응형 375px ~ 1440px 레이아웃 깨짐 없음
